from rlGames.envs.games.tank_battle.constants import GlobalConstants
from rlGames.envs.games.tank_battle.manager import ResourceManager
from rlGames.envs.games.tank_battle.map import ConditionalMap
from rlGames.envs.games.tank_battle.stages import StageMap
from rlGames.envs.games.tank_battle.sprites import TankSprite, BulletSprite, WallSprite, BaseSprite, ExplosionSprite
from rlGames.envs.games.utils import Utils
import numpy as np
import pygame
import os
import sys
...
from rlGames.utils.image import convert_rgb_to_grayscale_and_resize
...
class TankBattle(object):
    ...
    def __init__(self, render=False, speed=60, max_frames=100000, frame_skip=1,
        ...
        self.min_enemines = min_enemines
    ...
    def __bullets_update(self):
        for bullet in self.bullets_player:
            ...
            for enemy in enemies_hit:
                ...
                is_hit = True
                if len(self.enemies) ==  self.min_enemines:
                    self.__generate_enemies(self.num_of_enemies - self.min_enemines)
                break
            if is_hit:
                continue
    ...
    def __print_info(self):
        if self.is_debug:
            if self.frames_count % self.log_freq == 0:
                ...
                print("number of enemies left", len(self.enemies))
...
def machine_control(two_players=False):
    ...
    game = TankBattle(render=True, player1_human_control=False, player2_human_control=False, two_players=two_players,
                      speed=60, debug=False, frame_skip=5, using_map=False)
    scores = []
    scores_p1 = []
    scores_p2 = []
